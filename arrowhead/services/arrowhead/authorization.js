...
export async function intracloudAuthorization(authorizationRequestForm){
  console.log('Intracloud Authorization asking')
  return new Promise(async (resolve, reject) => {
    const authorizationAddress = coreSystemInfo && coreSystemInfo.authorization ? `${config.serverSSLEnabled ? 'https' : 'http' }://${coreSystemInfo.authorization.address}:${coreSystemInfo.authorization.port}/authorization` : null
      if(!authorizationAddress){
        return reject('No address for Authorization')
    }
    networkService.post(authorizationAddress + '/mgmt/intracloud', authorizationRequestForm)
      .then((response) => {
        fs.writeFileSync(path.resolve(__dirname + 'authorizationResponse.txt'), response)
...
export async function getPublicKey(){
  console.log('Ask for public key')
  return new Promise(async (resolve, reject) => {
    const authorizationAddress = coreSystemInfo && coreSystemInfo.authorization ? `${config.serverSSLEnabled ? 'https' : 'http' }://${coreSystemInfo.authorization.address}:${coreSystemInfo.authorization.port}/authorization` : null
      if(!authorizationAddress){
        return reject('No address for Authorization')
